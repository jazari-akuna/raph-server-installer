...
- **Camouflage naming.** Public DNS, file paths, container names, systemd units, and script names never use the words `vpn`, `wireguard`, `amnezia`, `tunnel`, `stealth`, `vault`, `tailscale`, `hysteria`, or upstream project names (`nextcloud`, `portainer`). Use the neutral vocabulary defined in the plan: `ingress`, `console`, `cloud`, `gw0`, `qedge`, `mesh`, `store`. This is defense-in-depth against passive observation, not against a shell-level attacker — package install names are out of scope. See ADR-002 in `docs/architecture-decisions.md`.
...
- **Encryption is at-rest via the VPS provider's full-disk encryption.** User data lives in plaintext on the host's encrypted filesystem (Layerstack FDE on the boot volume) under `/srv/store/cloud-data/<user>/`; per-user isolation is application-layer (Nextcloud ACLs + per-user data dir). Per-user LUKS was removed per ADR-004; do not re-introduce it. Nextcloud Server-side Encryption (`occ app:enable encryption`) is the documented opt-in if cold-disk-from-host-image protection is needed later.
- **Cloud uses OIDC, not forward-auth.** The `cloud` service (Nextcloud) authenticates via the Authelia OIDC `cloud` client. The Authelia rule for `cloud.${DOMAIN}` is `bypass` and Nextcloud owns the session. Do NOT mix OIDC and forward-auth on the same hostname — see ADR-003.
- **Persistent state lives on bind-mounts under `/srv/store/<stack>-*`.** ADR-001. Backup is one rsync per box plus a `pg_dump` per Postgres-backed stack. Do not put backup-relevant state in named volumes.
...
- **Backups are pulled from a workstation, not pushed from the server.** The server holds no backup credentials. The `pg_dump` + `rsync` recipe in `docs/backups.md` runs from the operator's laptop. Do not put a backup tool on the server.
